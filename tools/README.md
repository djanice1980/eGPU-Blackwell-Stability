# tools/

Small helpers that came out of the investigations in `docs/`. All user-level unless noted.

| Script | Purpose |
|---|---|
| `display-rescue` | Disable/re-enable every connected non-eDP output (forces a fresh modeset). Recovers an external display that stayed dark after DPMS wake. Bind it to a global shortcut — on Plasma 6 via KGlobalAccel DBus (`doRegister` + `setShortcut`) or System Settings → Shortcuts → Add Command. |
| `dpms-cycle.sh [N]` | Deterministic DPMS wake test: N blank/wake cycles with a y/n prompt per cycle, logged with kernel, `amdgpu.dcdebugmask`, mode and HDR state — the evidence format used in `docs/dpms-wake-bug-report.md`. |
| `greeter-display-diag.sh` | (sudo) Dumps the **plasmalogin** greeter's own KWin output config and its KWin log lines for this boot — for "external display dark until login" questions. |
| `kwin-outputconfig-dump.py [path]` | Readable dump of any `kwinoutputconfig.json` (handles the list-of-sections schema that `greeter-display-diag.sh`'s parser does not): every output with connector/EDID/mode/HDR, every saved setup with lid state and per-output enabled/priority. `sudo` it against `/var/lib/plasmalogin/.config/kwinoutputconfig.json` for the greeter. |
| `greeter-hdmi-4k60.sh` | (sudo) Pins the greeter's entry for a specific monitor (edit the `edidIdentifier` prefix inside — it ships with the reference LG) to 3840x2160@60 / HDR off, with a backup. Use only if the greeter reliably fails to light a 4K120 FRL/DSC link at first modeset. |
| `99-usb4-tunnel-ports-awake.rules` | udev rule that pins the two AMD USB4 PCIe-tunnel root ports (`0000:00:01.1/.2` on the GZ302EA) to `power/control=on`, so tunnels can establish without waiting for something to wake a runtime-suspended port — a narrow replacement for the global `pcie_port_pm=off` flag. Also forces the NVIDIA eGPU (matched by vendor/class, any bus address) and its HDA function to `power/control=on`, overriding the shipped `71-nvidia.rules` that sets `auto` — see NVIDIA/open-gpu-kernel-modules#1228. Adjust the root-port addresses for other boards. |
| `hdmi-link-retry` + `hdmi-link-retry.service` | Workaround for amdgpu `enabling link N failed: 19` (`DC_FAIL_HDMI_FRL_LINK_TRAINING`) at DPMS/lock-screen wake on an HDMI 2.1 FRL link: a user service that follows the kernel journal and waits 8 s (the monitor's own HPD pulse came at +8 s) then forces a modeset via `display-rescue`, up to 3 rounds per episode. Works while the screen is locked, unlike the Meta+Shift+D shortcut. **Not in use on the reference host since Sep 10:** the dark wake turned out to be the TV not syncing to the first FRL stream after standby; a second modeset (any timing) does not reliably light it, so the service was removed. Kept as an option for sinks where a retry does help. Install: copy to `~/.local/bin/` and `~/.config/systemd/user/`, then `systemctl --user enable --now hdmi-link-retry.service`. |
| `gen3-cap/` (`install.sh`) | **Gen3 bridge cap + late NVIDIA load** (efenex's recipe from NVIDIA#979, adapted): a udev rule starts a oneshot unit on every NVIDIA GPU add that sets Target Link Speed = Gen3 + Hardware Autonomous Speed Disable on the port directly above the GPU **and on the GPU's own link register** (Sep 16: so the endpoint does not downshift to Gen1 at idle and retrain at every game launch, which is where the Sep 16 hard drop happened; verified honoured on the 5070 Ti: Gen3 at idle P8 with the driver bound), retrains, then `modprobe`s nvidia; a twin unloads nvidia when the GPU leaves the bus so the boot-time tunnel rebuild is re-capped before the GSP boots. nvidia is blacklisted for udev autoload only (explicit modprobe works). Never retrains under a live driver. Cold boot only. Touches nothing but that one port and the nvidia modules. `sudo bash tools/gen3-cap/install.sh` / `--remove`; status: `sudo nvidia-egpu-cap-and-load --status`. |
| `egpu-display-test.sh W×H@R [hdr]` | Ladder test for driving an external display FROM the eGPU. Refuses to run unless clocks are locked (P0, gr>=1900), waits for KWin to expose the NVIDIA connector, resolves the mode by id from KWin's own list, **enables** the output, verifies the applied mode, and disables the eGPU output on a mode mismatch or the first Xid/loss line. Result on the reference host (Sep 6): 1080p60 SDR stable 12 min; 4K60 SDR lights clean then hard-freezes ~5 min into load. Escape: pull HDMI from the eGPU, not the TB cable. |
| `manager-test/manager-test-prep.sh` | Snapshot-protected test harness for DamianKA1993/blackwell-egpu-manager on this host: creates a snapper snapshot published to the Limine menu, moves our DPM modprobe file and root-port udev pin aside (copies kept), prints the run order. `manager-test-restore.sh` reverses it. |
| `manager-test/manager-test-log.sh <label>` | Appends a fingerprint (manager state, 5786/NVIDIA PCI tree, USB4 links, root-port PM, nvidia params/clocks, relevant journal) to `~/egpu-manager-test-<date>.log`. |
| `manager-test/manager-mode3-retry.sh` | (sudo) The corrected Mode 3 attach: rescans, checks BAR0 via sysfs, and if unassigned removes the 5786 upstream port, waits 3/8/15 s for the GPU-port link, rescans the root port again; then finishes the manager's own modeset/persistence/clock-lock steps. Never runs `fuser -k`. |

The DPMS wake problem on the reference system (AMD DCN 3.5.1 iGPU, HDMI behind a
DP-HDMI FRL PCON) turned out to be the display core's **IPS** (idle power states) exit
path; the fix is the kernel parameter `amdgpu.dcdebugmask=0x800` (`DC_DISABLE_IPS`). See
the runbook's DPMS section and the bug-report draft in `docs/`.

## hdmi-frl-lt-capture.sh (Sep 19)
Logs amdgpu's HDMI FRL link-training messages across one DPMS off/on cycle (turns the drm
driver debug class on only for the cycle) and summarises which way training failed. See the
runbook, Sep 19 section, for what each outcome means.

## amdgpu-frl-module/build.sh (Sep 19)
Builds only the amdgpu module with `kernel-patches/0001-*.patch` applied, against the running
kernel (srctree = CachyOS kernel source, O= a copy of the installed headers tree), installs it
under `/usr/lib/modules/<ver>/updates/` and rebuilds the initramfs. `--remove` reverts.
Stock module is never touched; a kernel update makes the override moot by itself.

