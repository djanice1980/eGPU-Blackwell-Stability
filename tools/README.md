...
| `hdmi-link-retry` + `hdmi-link-retry.service` | Workaround for amdgpu `enabling link N failed: 19` (`DC_FAIL_HDMI_FRL_LINK_TRAINING`) at DPMS/lock-screen wake on an HDMI 2.1 FRL link: a user service that follows the kernel journal and forces one modeset via `display-rescue` 3 s after the message (20 s cooldown). Works while the screen is locked, unlike the Meta+Shift+D shortcut. Install: copy to `~/.local/bin/` and `~/.config/systemd/user/`, then `systemctl --user enable --now hdmi-link-retry.service`. |
